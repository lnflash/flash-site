...
function msgBorderCalcs(cont) {
  ...
  const borderLength = mPerimeter * 0.5; // length of the moving line
  const borderGap = mPerimeter - borderLength; // gap between dashes to ensure only 1 line is visible
  const dashOffset = mWidth * 0.65 * -1; // position the dash line
...
// Adjust ticker attribute
function addBrackets() {
  const oldSpan = document.querySelector("#gobtc-widget-price span");
  if (span !== null) {
    const span = document.createElement("span");
    span.style.cssText = "font-size: 9px; color:#000; font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;";
    const newSpan = `
      <span style="font-size: 9px; color:#000; font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
        Powered by 
        <a href="https://gobitcoin.io/">Gobitcoin.io</a>
      </span>`;
