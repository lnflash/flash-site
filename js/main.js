...
function changeTab(activeTab) {
  ...
  let tabHeight =
    activeTab.querySelector(".accordion-body").scrollHeight + "px";
...
function toggleCredits() {
  creditsContent.classList.contains("open")
    ? creditsContent.classList.remove("open")
    : creditsContent.classList.add("open");
}
...
window.onload = () => {
  ...
  cvsTch.onpointerdown = (e) => {
    ...
    touch = true;
    tchTarget = {
      x: e.clientX - cvsTch.getBoundingClientRect().left,
      y: e.clientY - cvsTch.getBoundingClientRect().top,
    };
    ...
    cvsTch.onpointermove = (e) => {
      ...
      if (
        x > 0 &&
        e.clientX < cvsTch.getBoundingClientRect().right &&
        y > 0 &&
        e.clientY < cvsTch.getBoundingClientRect().bottom
      ) {
        tchTarget = { x: x, y: y };
      } else {
